...
const basicFetch = async (endpoint: string, config?: {}) => {
    if(config){
        const req = await fetch(`${API_BASE}${endpoint}`, config)
...
export default {
    ...
    getMovieInfo: async (movieId: number, type: string) => {
        ...
        if(movieId) {
            switch(type) {
                case 'movie':
                    info = await basicFetch(`/movie/${movieId}&language=pt-Br`, config)
                    break
                case 'tv':
                    info = await basicFetch(`/tv/${movieId}&language=pt-Br`, config)
                    break
                default:
                    break
            }
